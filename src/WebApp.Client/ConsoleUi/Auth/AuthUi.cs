...
namespace WebApp.Client.ConsoleUi.Auth
{
        ...
        : IAuthUi
    {
        ...
        private async Task RegisterAsync()
        {
            ...
            _console.WriteLine(AppConstants.Titles.Register);
            ...
            var password = Input.RequiredString(AppConstants.Prompts.Password);
            ...
            _console.WriteLine(AppConstants.Messages.RegisteredAndLoggedIn);
        }

        private async Task LoginAsync()
        {
            _console.WriteLine(string.Empty);
            _console.WriteLine(AppConstants.Titles.Login);

            var email = Input.RequiredString(AppConstants.Prompts.Email);
            var password = Input.RequiredString(AppConstants.Prompts.Password);
            await login.ExecuteAsync(email, password, CancellationToken.None);
            _console.WriteLine(AppConstants.Messages.LoggedIn);
        }
    }
}
